...
- Session lineage stops at depth 8: `create_session`, `send_later`, and `create_trigger` all refuse there ("caller session is at lineage depth 8 (limit 8)"). A chain of sessions that each spawn their successor dies at the eighth; a session started from the UI is depth 0, its child depth 1.
- Only the newest subscriber to a PR appears to receive its events: a session waiting on a PR for a message from the session that subscribed after it never woke (inferred from timestamps, not documented). Design handoffs so the old session never waits for an event.
- A one-shot `create_trigger` with `persistent_session_id` set to another session of the same account wakes that session with the trigger's prompt about 20 seconds after `run_once_at`. It needs no GitHub Actions, so it can stand in for a comment relay. Verified from a top-level session only.
- A session fired by a Routine with `create_new_session_on_fire` starts a fresh lineage (tag `config:routine-lineage-none`), but a Routine created by `create_trigger` runs the default model with no repository attached, and it can pass no connectors the calling session lacks. Set the model and repo in the claude.ai Routines UI instead.
- A comment relay built on GitHub Actions dies with Actions: when account billing fails, every job fails in about 2 seconds with "The job was not started because recent account payments have failed", so CI and the relay stop together without any other signal.
- Every comment a session posts carries `performed_via_github_app.slug == "claude"`, including ones posted through `curl`; the human's own comments carry none. When every session posts as the same user, that field is the only reliable way to tell the human's approval from an agent's.
- `ListAgents` in a cloud session lists no other cloud sessions, so `SendMessage` cannot reach them from there.
