...
3. **Route each fact to the right doc.** When an area uses the standard doc
   set:
   - architecture, phases, and domain model → the design doc;
   - facts about external systems (APIs, warehouses, vendors) → the
     integrations doc;
   - regulatory or policy rules → the compliance doc;
   - decisions → the decision log (append a new `D<n>`; never rewrite an
     accepted decision, see step 5);
   - risks and unknowns → the open-questions doc (mark resolved items ✅ with
     the answer inline).
...
5. **Contradictions are decisions, not edits.** If your change conflicts with
   an accepted entry in the area's decision log, do not silently rewrite the
   decision. Add the conflict to the open-questions doc and flag it in your
   reply; the decision log only changes with explicit sign-off.
