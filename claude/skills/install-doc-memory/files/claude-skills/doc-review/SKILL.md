...
2. **Establish the doc's sources of truth.** From `docs/doc-map.tsv`, find the
   ...
   and which rest on external sources (vendor docs, regulatory guidance,
   internal documents, chat threads).
3. **Verify claim by claim.** For each factual claim, path, command, and
   ...
   - **unverifiable from the repo** → leave it, but if it looks doubtful, add
     it to the area's open-questions doc with what would confirm it.
   ...
   over time, keep the newer claim and move the question to the area's
   open-questions doc.
