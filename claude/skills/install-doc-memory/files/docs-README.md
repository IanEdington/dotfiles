...
as the code. It exists because every collaborator, human or AI agent, starts
from the same place: sessions are ephemeral, containers are thrown away, and
only what is committed persists. Knowledge that lives in one person's head,
one chat transcript, or one chat thread is lost to everyone else; knowledge
that lives here is loaded on demand by whoever needs it.
...
present and must track it. Snapshots (for example vendor API specs saved into
`docs/`) are point-in-time artifacts: a review does not edit them, it asks
whether the snapshot is old enough to re-request from the source. Both kinds
carry freshness frontmatter; only the review action differs.
