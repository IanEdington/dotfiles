...
#### Import script
	# import_script () {
	# 	if [ -f "$HOME/.$1.bash" ]; then
	# 		script_path="$HOME/.$1.bash";
	# 	else if [ -f "$HOME/$1.bash" ]; then
	# 		script_path="$HOME/$1.bash";
	# 	else if [ -f "$HOME/.config/bash/$1.bash" ]; then
	# 		script_path="$HOME/.config/bash/$1.bash";
	# 	else if [ -f "$HOME/.config/bash/$1" ]; then
	# 		script_path="$HOME/.config/bash/$1";
	# 	fi
	# }
...
#### homebrew
	export HOMEBREW_GITHUB_API_TOKEN="2b7441d9454fdd5d946c7023f12bee2b6d0cc5bf"
...
#### Edit the .profile file
	...
	alias .bookmarks='$EDITOR2 ~/.config/bash/bookmarks.bash'

#### Pretty Prompt:
#	export PS1='\[\033[0;33m\]\u\[\033[0;35m\]@\[\033[0;33m\]\h\[\033[0;35m\]$\[\033[00m\] '
#### Set Paths (in revers order of importance)
...
# 2. Standard Command defaults flags and extra functionality
...
#### 4.2 Git
###### Pull default git repo on launch
	if [ -f "$HOME/.git_repo_to_pull.bash" ]; then
	    source ~/.git_repo_to_pull.bash
	    alias pull='source ~/.git_repo_to_pull.bash'
	fi
###### Alias
	alias gs="git status"
...
# Taken from Ubuntu on Digital Ocean
